...
Ext.extend(Ext.ux.grid.GridHeaderFilters, Ext.util.Observable,
{
  ...
  init:function(grid){
    ...
    Ext.apply(this.grid, {
      ...
      refreshHeaderFilters: function(cm) {
        (function() {
          this.headerFilters.destroyFilters();
          this.headerFilters.configure(cm);
          this.headerFilters.renderFilters();
        }.defer(200, this));
      }
    });
  ...
  renderFilterContainer: function(columnId, fcc){
    ...
    if(Ext.isGecko){  //Patch for field text selection on Mozilla
  ...
  ColResize: function(index, iWidth){
    ...
    if(cnt){
      var fields = cnt.findBy(function(cmp){return !Ext.isEmpty(cmp.filterName);});
      if(!Ext.isEmpty(fields)){
        for(var i=0;i<fields.length;i++){
          fields[i].setWidth(filterW);
        }
      }
